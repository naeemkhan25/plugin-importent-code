...
function save_select_post($post_id){
    ...
    $select_term=$_POST['categoryTerm'];

    if($select_post>0){
        update_post_meta($post_id,'select_post_meta',$select_post);
    }
    if($select_term>0){
        update_post_meta($post_id,'select_terms_meta',$select_term);
    }
    return $post_id;
...
echo "</br>";
    ...
    wp_reset_query();
$term_data=get_post_meta($post->ID,'select_terms_meta',true);
echo $term_data;
$termsOption='';
   ...
   foreach ($_terms as $term){
       $selected_term='';
       if($term_data==$term->term_id){
           $selected_term='selected';
       }
       $termsOption.=sprintf('<option %s value="%s">%s</option>',$selected_term,$term->term_id,$term->name);
   }

    $label=__("Choose a post",'post-Tax-Meta');
    $label2=__("Choose a category Term",'post-Tax-Meta');
...
<br/>
<div>
 <label for="post_meta">{$label2}</label>

<select   id="post_meta"  name="categoryTerm">
